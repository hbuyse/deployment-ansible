...
class CallbackModule(CallbackBase):
    ...
    def __init__(self):
        ...
        # Tracks whether a "task running" line is currently on screen
        # and waiting to be overwritten by the final result line.
        self._line_in_progress = False
        # Only attempt to overwrite lines (carriage return + clear) when
        # stdout is an actual terminal - doing this on a redirected file
        # or CI log would just dump raw control characters.
        try:
        ...
        self.show_timestamps = (
            os.environ.get("ANSIBLE_PRETTIFY_SHOW_TIMESTAMPS", "false").lower()
            == "true"
        )
        self.show_timing = (
            os.environ.get("ANSIBLE_PRETTIFY_SHOW_TIMING", "true").lower() == "true"
        )
    ...
    def _clear_current_line(self):
        """Erase the in-progress 'running' line so the final result can be
        written in its place instead of appending a new line."""
        if self._line_in_progress:
            # \r moves cursor to column 0, \033[K clears from cursor to EOL
            sys.stdout.write("\r\033[K")
            sys.stdout.flush()
            self._line_in_progress = False

    def _print_role_header_if_needed(self, task):
        """Print the role header once, the first time we see a new role.
        Shared between the 'running' line and the final result line so it
        only ever appears once regardless of which one triggers it first."""
        current_role = self._get_task_role(task)
    ...
    def _print_running(self, task):
        """Print the task name as soon as it starts running, without a
        trailing newline, so it can be overwritten once the result is
        known. Falls back to doing nothing when stdout isn't a TTY (e.g.
        redirected to a log file), since overwriting doesn't make sense
        there - the final line will simply be printed normally."""
        if not self._is_tty:
            return

        self._print_role_header_if_needed(task)
        ...
        if not task_name or not task_name.strip():
            task_name = f"[{getattr(task, 'action', 'task')}]"

        max_width = self._get_terminal_width()
        max_task_width = max(
            max_width - self.PREFIX_SPACE, self.MIN_TERMINAL_WIDTH - self.PREFIX_SPACE
        )
        display_name = task_name
        if len(display_name) > max_task_width:
            display_name = display_name[: max_task_width - 3] + "..."

        symbol = self._colorize(self.symbols["running"], self.colors["running"])
        line = f"  {symbol} {color_text(display_name, self.colors['task_name'])}"
    ...
    def _print_result(self, result, status):
        ...
        # If a "running" line for this task is still on screen, wipe it -
        # the result line below replaces it entirely rather than being
        # appended after it.
        ...
        # Check if we need to show a role header (no-op if already shown
        # when the "running" line for this task was printed)
        self._print_role_header_if_needed(result._task)
        ...
        if len(display_name) > max_task_width:
            ...
            break_chars = [
                ...
                "-",
                ":",
            ]  # Removed '.' and '/' to avoid bad path breaks
            best_break = -1
            ...
            search_end = max(
                25, max_task_width // 2
            )  # Increased minimum to avoid very short first lines
    ...
    def v2_playbook_on_task_start(self, task, is_conditional):
        ...
        self.last_task = task
        self._print_running(task)

    def v2_playbook_on_handler_task_start(self, task):
        """Called when a handler task starts (separate hook from regular
        tasks in Ansible) - show the same 'running' line for consistency."""
        self.task_start_time = time.time()
        self.last_task = task
        self._print_running(task)
    ...
    def v2_playbook_on_stats(self, stats):
        ...
        for host in hosts:
            ...
            if summary["ok"] > 0:
                status_parts.append(
                    self._colorize(
                        f"✓ {summary['ok']} successful", self.colors["success"]
                    )
                )
            if summary["changed"] > 0:
                status_parts.append(
                    self._colorize(
                        f"~ {summary['changed']} changed", self.colors["changed"]
                    )
                )
            if summary["failures"] > 0:
                status_parts.append(
                    self._colorize(
                        f"✗ {summary['failures']} failed", self.colors["failed"]
                    )
                )
            if summary["unreachable"] > 0:
                status_parts.append(
                    self._colorize(
                        ...
                        self.colors["unreachable"],
                    )
                )
            if summary["skipped"] > 0:
                status_parts.append(
                    self._colorize(
                        f"→ {summary['skipped']} skipped", self.colors["skipped"]
                    )
                )
        ...
        if "deploy" in playbook_name:
            completion_msg = (
                f"🚀 Deployment completed successfully in {total_time:.1f}s"
            )
        elif "provision" in playbook_name:
            completion_msg = (
                f"⚙️ Provisioning completed successfully in {total_time:.1f}s"
            )
        else:
